...
async function func614(this: any) {
        ...
        if (Gvar.var_83[Gvar.var_673].Var0 == 128 || Gvar.var_83[Gvar.var_673].Var0 == 171) {
            await Func.setMessage("「スタープラチナ・ザ・ワールド」！！", "", 8, false, false, false);
        }
        else {
            await Func.setMessage("「ザ・ワールド」！！", "時よ止まれ！", 8, false, false, false);
        }
        Adap.DSPLAY(185);
        ...
        if (Gvar.var_83[Gvar.var_673].Var0 == 128) {
            await Func.setMessage("ひさしぶりに……", "時をとめられたぜ", 8, false, false, false);
        }
        // Ver0.1403にて追加。No = 171 離婚した承太郎。
        if (Gvar.var_83[Gvar.var_673].Var0 == 171) {
            await Func.setMessage("時は最大で5秒ほど", "止まっている", 8, false, false, false);
        }
        else {
            await Func.setMessage("フフフフフ…", "これが「世界」だ…", 8, false, false, false);
        }
        Gvar.var_240 = 0;
        ...
        if (Gvar.nouryoku_disc_id == 100 || Gvar.nouryoku_disc_id == 111 || Gvar.nouryoku_disc_id == 398) {
            ...
            Gvar.var_271 = 0; // エフェクト "キラキラ" 表示フラグOFF
            Gvar.var_1493 = 0;
            await Func.setMessage("ﾃﾞｨｱﾎﾞﾛ「おれが時を止めた…」", "", 12, true, false, false);
            await Func.AutoDraw(2);
            await Func.setMessage("どんな気分だ？", "動けねえのに背後に立たれる気分はよ？", 12, true, false, false);
            await Func.AutoDraw(2);
            Gvar.var_153 = 1;
            return;
        }
