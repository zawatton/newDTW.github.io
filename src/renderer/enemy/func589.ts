...
async function func589(this: any) {
        ...
        if (Gvar.var_2830 == 0) {
            ...
            if (Gvar.var_2352 == 0) {
                await Func.setMessage("ﾁｮｺﾗｰﾀはｾｯｺを呼んだ！", "", 8, false, false, false);
            }
            if (Gvar.var_2352 == 1) {
                await Func.setMessage("ﾁｮｺﾗｰﾀはｸﾞﾘｰﾝﾃﾞｨを発現させた！", "", 8, false, false, false);
            }
            ...
            if (Gvar.var_2110 == 0) {
                if (Gvar.var_2352 == 0) {
                    await Func.setMessage("しかしｾｯｺは来なかった…", "", 8, false, false, false);
                }
                if (Gvar.var_2352 == 1) {
                    await Func.setMessage("しかしｽﾀﾝﾄﾞを出せなかった…", "", 8, false, false, false);
                }
                Gvar.var_2197 = 1;
                return;
            }
            ...
            if (Gvar.var_2352 == 0) {
                await Func.setMessage("ヤツが死ぬところを撮影して来いッ！", "", 8, false, false, true);
            }
            if (Gvar.var_2352 == 1) {
                await Func.setMessage("低いところに降りられないぞ…", "", 8, false, false, true);
            }
            Gvar.var_2197 = 1;
        }
        if (Gvar.var_2830 == 1 || Gvar.var_2830 == 2) {
            ...
            if (Gvar.var_1565 >= 1) {
                await Func.setMessage("ﾁｮｺﾗｰﾀ「甘いの３つ欲しいのかッ！？」", "", 8, false, false, true);
                ...
                Gvar.var_271 = 0; // エフェクト "キラキラ" 表示フラグOFF
                Gvar.var_1566 = 0;
                await Func.setMessage("セッコ「ガリガリガリガリガリガリガリ」", "", 8, false, false, false);
                Adap.DSPLAY(196);
                for (let cnt3 = 0; cnt3 < 10; ++cnt3) {
                    Gvar.var_411 = 2;
                    await Func.func337(); // メッセージ表示処理(自動)
                ...
                Adap.DSPLAY(182);
                await Func.setMessage("近くのセッコの攻撃力が上がった！", "", 8, false, false, false);
                await Func.AutoDraw(9);
                Gvar.var_2197 = 1;
            }
        }
