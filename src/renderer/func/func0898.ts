    ...
    if (Gvar.key_Z_on == 1 || Gvar.key_A_on == 1) {
        if (Gvar.var_546 == 1) {// 二つある選択肢の上側を選んだ時のフロー
            ...
            if (Gvar.var_3163 == 2) {
                ...
                Gvar.var_548 = 0; // 選択肢処理フラグOFF
                await Func.func051(); // キー入力待ち処理
                await Func.func791(); // アバッキオ茶のダメージセリフ
                return;
            }
            if (Gvar.var_3163 == 3) {
                Gvar.var_3163 = 0;
            ...
            if (Gvar.var_3199 == 1) {
                Gvar.var_3199 = 0;
                Gvar.var_548 = 0; // 選択肢処理フラグOFF
                await Func.func0886(); // ダンジョン「レクイエムの大迷宮」へ入った時の初期化処理
                return;
            }
            if (Gvar.var_3200 == 1) {
                Gvar.var_3200 = 0;
                Gvar.var_548 = 0; // 選択肢処理フラグOFF
                await Func.func0888(); // ダンジョン「ディアボロの試練」へ入った時の初期化処理
                return;
            }
            if (Gvar.var_3201 == 1) {
                Gvar.var_3201 = 0;
                Gvar.var_548 = 0; // 選択肢処理フラグOFF
                await Func.func0893(); // ダンジョン「一巡後の世界」へ入った時の初期化処理
                return;
            }
            if (Gvar.var_3501 == 1) {
                Gvar.var_3501 = 0;
                Gvar.var_548 = 0; // 選択肢処理フラグOFF
                await Func.func0895(); // ダンジョン「鉄獄(鉄の牢獄)」へ入った時の初期化処理
                return;
            }
            if (Gvar.var_3192 == 1) {
                Gvar.var_3192 = 0;
    ...
    if (Gvar.key_Z_on == 1 || Gvar.key_A_on == 1) {
        if (Gvar.var_546 == 0) {// 二つある選択肢の下側を選んだ時のフロー
            ...
            if (Gvar.var_3136 == 1) {
                ...
                Gvar.var_548 = 0; // 選択肢処理フラグOFF
                await Func.func051(); // キー入力待ち処理
                await Func.func751(); // ペッシに話しかけて仲間にするのを拒んだ際の動作処理
                return;
            }
            if (Gvar.var_547 == 1) {
                Gvar.var_547 = 0;
