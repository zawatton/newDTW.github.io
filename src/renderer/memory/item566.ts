...
async function item566(this: any) {
        ...
        if (Gvar.var_1073 == 0) {
            await Func.setMessage("「健康の秘訣は睡眠8時間半と",
                                   "  毎朝一杯の尿療法」", 7, false, false, true);
        }
        if (Gvar.var_1073 == 1) {
            await Func.setMessage("「40年前",
                                   "  わしは輝いておったんじゃあああああ」", 7, false, false, true);
        }
        if (Gvar.var_1073 == 2) {
            await Func.setMessage("「教祖復活じゃああああああ",
                                   "  あああああああああああああ」", 7, false, false, true);
        }
        if (Gvar.var_1073 == 3) {
            await Func.setMessage("「完璧なる【守りの方角】",
